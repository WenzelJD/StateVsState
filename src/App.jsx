...
let audioUnlocked=false;const acRef={current:null};let soundEnabled=true;
...
function playTick(){if(!soundEnabled)return;try{const c=ac(),o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='sine';o.frequency.value=1800+Math.random()*800;g.gain.setValueAtTime(0.12,c.currentTime);g.gain.exponentialRampToValueAtTime(0.001,c.currentTime+0.04);o.start(c.currentTime);o.stop(c.currentTime+0.04);}catch(e){}}
function playDrumRoll(){if(!soundEnabled)return;try{const c=ac(),dur=1.5,t=c.currentTime,m=c.createGain();m.gain.setValueAtTime(0.05,t);m.gain.linearRampToValueAtTime(0.35,t+dur*0.8);m.gain.exponentialRampToValueAtTime(0.001,t+dur);m.connect(c.destination);for(let i=0;i<40;i++){const ht=t+(i/40)*dur,hd=0.03,hb=c.createBuffer(1,Math.floor(c.sampleRate*hd),c.sampleRate),hD=hb.getChannelData(0);for(let j=0;j<hD.length;j++)hD[j]=(Math.random()*2-1)*Math.pow(1-j/hD.length,3);const hs=c.createBufferSource();hs.buffer=hb;const hg=c.createGain();hg.gain.value=0.4+Math.random()*0.2;const hp=c.createBiquadFilter();hp.type='bandpass';hp.frequency.value=200+Math.random()*100;hp.Q.value=1;hs.connect(hp);hp.connect(hg);hg.connect(m);hs.start(ht+(Math.random()-0.5)*0.01);hs.stop(ht+hd+0.01);}}catch(e){}}
function playImpact(){if(!soundEnabled)return;try{const c=ac(),o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='sine';o.frequency.setValueAtTime(150,c.currentTime);o.frequency.exponentialRampToValueAtTime(30,c.currentTime+0.3);g.gain.setValueAtTime(0.6,c.currentTime);g.gain.exponentialRampToValueAtTime(0.001,c.currentTime+0.4);o.start(c.currentTime);o.stop(c.currentTime+0.4);const b=c.createBuffer(1,c.sampleRate*0.06,c.sampleRate),d=b.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=(Math.random()*2-1)*Math.pow(1-i/d.length,3);const s=c.createBufferSource();s.buffer=b;const g2=c.createGain();g2.gain.setValueAtTime(0.5,c.currentTime);g2.gain.exponentialRampToValueAtTime(0.001,c.currentTime+0.08);s.connect(g2);g2.connect(c.destination);s.start(c.currentTime);s.stop(c.currentTime+0.08);}catch(e){}}
function playWhoosh(){if(!soundEnabled)return;try{const c=ac(),bs=c.sampleRate*0.4,b=c.createBuffer(1,bs,c.sampleRate),d=b.getChannelData(0);for(let i=0;i<bs;i++)d[i]=(Math.random()*2-1);const s=c.createBufferSource();s.buffer=b;const bp=c.createBiquadFilter();bp.type='bandpass';bp.frequency.setValueAtTime(800,c.currentTime);bp.frequency.exponentialRampToValueAtTime(3000,c.currentTime+0.15);bp.frequency.exponentialRampToValueAtTime(200,c.currentTime+0.35);bp.Q.value=2;const g=c.createGain();g.gain.setValueAtTime(0.4,c.currentTime);g.gain.exponentialRampToValueAtTime(0.001,c.currentTime+0.4);s.connect(bp);bp.connect(g);g.connect(c.destination);s.start(c.currentTime);s.stop(c.currentTime+0.4);}catch(e){}}
function playCountdownBeep(high){if(!soundEnabled)return;try{const c=ac(),o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='square';o.frequency.value=high?880:440;g.gain.setValueAtTime(0.2,c.currentTime);g.gain.exponentialRampToValueAtTime(0.001,c.currentTime+(high?0.3:0.15));o.start(c.currentTime);o.stop(c.currentTime+(high?0.3:0.15));}catch(e){}}
function playDing(){if(!soundEnabled)return;try{const c=ac(),t=c.currentTime;[880,1108,1320].forEach((f,i)=>{const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='sine';o.frequency.value=f;g.gain.setValueAtTime(0,t+i*0.08);g.gain.linearRampToValueAtTime(0.25,t+i*0.08+0.02);g.gain.exponentialRampToValueAtTime(0.001,t+i*0.08+0.6);o.start(t+i*0.08);o.stop(t+i*0.08+0.6);});}catch(e){}}
function playBuzzer(){if(!soundEnabled)return;try{const c=ac(),t=c.currentTime;const o1=c.createOscillator(),o2=c.createOscillator(),g=c.createGain();o1.connect(g);o2.connect(g);g.connect(c.destination);o1.type='sawtooth';o1.frequency.value=120;o2.type='square';o2.frequency.value=123;g.gain.setValueAtTime(0.35,t);g.gain.setValueAtTime(0.35,t+0.35);g.gain.exponentialRampToValueAtTime(0.001,t+0.5);o1.start(t);o1.stop(t+0.5);o2.start(t);o2.stop(t+0.5);}catch(e){}}
...
function playFanfareSynth(){if(!soundEnabled)return;try{const c=ac(),t=c.currentTime,m=c.createGain();m.gain.setValueAtTime(0.3,t);m.gain.setValueAtTime(0.3,t+1.8);m.gain.exponentialRampToValueAtTime(0.001,t+2.5);m.connect(c.destination);[{f:523,s:0,d:0.15},{f:659,s:0.15,d:0.15},{f:784,s:0.3,d:0.15},{f:1047,s:0.5,d:0.4},{f:784,s:1.0,d:0.12},{f:880,s:1.12,d:0.12},{f:1047,s:1.3,d:0.6}].forEach(n=>{[1,2,3,4].forEach((h,hi)=>{const o=c.createOscillator(),g=c.createGain();o.type=hi===0?'sawtooth':'sine';o.frequency.value=n.f*h;const vol=0.25/(h*h);g.gain.setValueAtTime(0,t+n.s);g.gain.linearRampToValueAtTime(vol,t+n.s+0.02);g.gain.setValueAtTime(vol,t+n.s+n.d*0.7);g.gain.exponentialRampToValueAtTime(0.001,t+n.s+n.d+0.1);o.connect(g);g.connect(m);o.start(t+n.s);o.stop(t+n.s+n.d+0.15);});});}catch(e){}}
function playFanfare(){if(!soundEnabled)return;if(victoryBuffer){try{const c=ac(),src=c.createBufferSource();src.buffer=victoryBuffer;src.connect(c.destination);src.start(0);return;}catch(e){}}playFanfareSynth();}
function playApplause(big){if(!soundEnabled)return;try{const c=ac(),dur=big?5:3.5,t=c.currentTime,m=c.createGain();m.gain.setValueAtTime(0.001,t);m.gain.linearRampToValueAtTime(big?0.5:0.35,t+0.4);m.gain.setValueAtTime(big?0.5:0.35,t+dur*0.55);m.gain.linearRampToValueAtTime(big?0.25:0.15,t+dur*0.8);m.gain.exponentialRampToValueAtTime(0.001,t+dur);m.connect(c.destination);(big?[500,1000,1800,3200]:[800,1400,2800]).forEach((f,i)=>{const bs=Math.floor(c.sampleRate*dur),b=c.createBuffer(1,bs,c.sampleRate),d=b.getChannelData(0);for(let j=0;j<bs;j++)d[j]=(Math.random()*2-1);const s=c.createBufferSource();s.buffer=b;const bp=c.createBiquadFilter();bp.type='bandpass';bp.frequency.value=f;bp.Q.value=0.4+i*0.15;const g=c.createGain();g.gain.value=0.22-i*0.04;s.connect(bp);bp.connect(g);g.connect(m);s.start(t);s.stop(t+dur);});}catch(e){}}
...
export default function App(){
  ...
  const toggleAudio=(v)=>{setAudioOn(v);soundEnabled=v;if(!v){stopTTS();window.speechSynthesis?.cancel();}};
      ...
      <div style={{display:'flex',gap:14,alignItems:'flex-start',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          {!champion&&<div style={{marginTop:6,display:'flex',justifyContent:'flex-end',alignItems:'center',gap:12}}>
              <label style={{display:'flex',alignItems:'center',gap:6,cursor:'pointer',fontSize:12,color:'#94a3b8',fontWeight:600,userSelect:'none'}} onClick={e=>e.stopPropagation()}><input type="checkbox" checked={audioOn} onChange={e=>toggleAudio(e.target.checked)} style={{width:16,height:16,accentColor:'#f59e0b',cursor:'pointer'}}/>{audioOn?'🔊':'🔇'} Audio</label>
