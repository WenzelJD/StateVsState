...
function USMap({features,territories,populations,active,fighter1,fighter2,winner,loser,hoveredState,setHoveredState,validTargets}){
  if(!features.length)return <div style={{color:'#64748b',textAlign:'center',padding:40}}>🗺️ Loading map...</div>;
  const hasTargets=validTargets&&validTargets.length>0;
  return(<div style={{position:'relative',background:'rgba(15,23,42,0.5)',borderRadius:16,border:'2px solid rgba(251,191,36,0.12)',overflow:'hidden'}}>
    <svg viewBox="0 0 975 610" style={{width:'100%',height:'auto',display:'block'}}>
      {features.map(f=>{const name=f.name;if(!name)return null;const owner=territories[name]||name;const col=STATE_COLORS[owner]||'#1e3a5f';const isF1=owner===fighter1||name===fighter1;const isF2=owner===fighter2||name===fighter2;const isW=owner===winner;const isL=loser&&owner===loser;const isH=hoveredState===name;const isTarget=hasTargets&&validTargets.includes(owner);const isOwned=hasTargets&&owner===fighter1;const dimmed=hasTargets&&!isTarget&&!isOwned&&!isF1&&!isF2&&!isW&&!isL;let fill=dimmed?'#1a1a2e':col;let stroke='rgba(255,255,255,0.15)',sw=0.8,filt='',op=dimmed?0.35:1;if(isW){stroke='#4ade80';sw=2.5;filt='url(#glowG)';}else if(isL){stroke='#f87171';sw=2.5;filt='url(#glowR)';}else if(isF1){stroke='#60a5fa';sw=2.5;filt='url(#glowB)';}else if(isF2){stroke='#f87171';sw=2;filt='url(#glowR)';}else if(isTarget){stroke='#fbbf24';sw=1.5;}else if(isH&&!dimmed){stroke='#fff';sw=2;}const c=f.centroid;const show=isF1||isF2||isW||isL||(isH&&!dimmed)||(isTarget&&!dimmed);return(<g key={name} onMouseEnter={()=>setHoveredState(name)} onMouseLeave={()=>setHoveredState(null)} style={{cursor:'default'}}><path d={f.path} fill={fill} stroke={stroke} strokeWidth={sw} filter={filt} opacity={op} style={{transition:'fill 0.8s,stroke 0.5s,opacity 0.5s'}}/>{show&&c&&<text x={c[0]} y={c[1]} textAnchor="middle" dominantBaseline="central" fill="#fff" fontSize={12} fontWeight="bold" style={{pointerEvents:'none',textShadow:'0 0 6px rgba(0,0,0,0.95)'}}>{ABBR[name]}</text>}</g>);})}
      <defs><filter id="glowB"><feDropShadow dx="0" dy="0" stdDeviation="5" floodColor="#3b82f6" floodOpacity="0.8"/></filter><filter id="glowR"><feDropShadow dx="0" dy="0" stdDeviation="5" floodColor="#ef4444" floodOpacity="0.8"/></filter><filter id="glowG"><feDropShadow dx="0" dy="0" stdDeviation="5" floodColor="#22c55e" floodOpacity="0.8"/></filter></defs>
    </svg>
    {hoveredState&&<div style={{position:'absolute',top:8,right:12,background:'rgba(0,0,0,0.8)',color:'#e2e8f0',padding:'6px 12px',borderRadius:8,fontSize:12,fontWeight:700,pointerEvents:'none',border:`2px solid ${STATE_COLORS[territories[hoveredState]]}`}}><div>{hoveredState}</div><div style={{fontSize:11,color:'#94a3b8'}}>Owner: {territories[hoveredState]===hoveredState?'Independent':territories[hoveredState]}</div><div style={{fontSize:11,color:'#fbbf24'}}>👥 Empire pop: {populations[territories[hoveredState]]||0}</div></div>}
  </div>);
}
    ...
    <div style={{minHeight:'100vh',background:'linear-gradient(150deg,#0f172a 0%,#1e1b4b 40%,#172554 100%)',color:'#e2e8f0',fontFamily:'system-ui,-apple-system,sans-serif',padding:12,boxSizing:'border-box',position:'relative',overflow:'hidden'}} onClick={unlockAudio}>
      ...
      <div style={{textAlign:'center',marginBottom:8}}>
        ...
        <div style={{fontSize:12,color:'#94a3b8',marginTop:2,fontWeight:600}}>
          {champion?'🎉 Grand Champion! 🎉':`Round ${round} · ${active.length} empires`}
      ...
      <div style={{display:'flex',gap:14,alignItems:'flex-start',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <USMap features={features} territories={territories} populations={populations} active={active} fighter1={mF1} fighter2={mF2} winner={winner} loser={loser} hoveredState={hov} setHoveredState={setHov} validTargets={w2}/>
          <div style={{margin:'8px 0',padding:'8px 12px',background:'rgba(0,0,0,0.3)',borderRadius:10,border:'1px solid rgba(251,191,36,0.1)'}}>
            <div style={{display:'flex',justifyContent:'space-between',fontSize:11,color:'#94a3b8',marginBottom:4,fontWeight:600}}><span>🏰 {active.length} empires</span><span>🗺️ {52-active.length} conquered</span></div>
